...
_RE_GOAL  = re.compile(r"^\s*goal\s+(?P<goal>.+)$", re.I)
_RE_EARN_1 = re.compile(r"^\s*earn\s+(?P<amount>[\d.]+)\s+(?P<token>\w+)\s*$", re.I)
_RE_EARN_2 = re.compile(r"^\s*earn\s+(?P<token>\w+)\s+(?P<amount>[\d.]+)\s*$", re.I)
...
def parse_goal(text: str) -> Optional[str]:
    m = _RE_GOAL.match(text or "")
    if not m: return None
    goal = m.group("goal").strip()
    return f"/goal {goal}"

def parse_earn(text: str) -> Optional[str]:
    m = _RE_EARN_1.match(text or "") or _RE_EARN_2.match(text or "")
    if not m: return None
    amt = m.group("amount"); tok = _norm(m.group("token"))
    return f"/earn {tok} {amt}"
